...
export default function ProfessorDashboardPage() {
  ...
  const QUICK_STATS = [
    ...
    { label: "Talabalar", value: String(stats.students || "124"), icon: <Users size={16} />, color: "#0D9373", chart: [80,90,85,95,92,88,94].map(v => ({ v })) },
    { label: "O'rt. diqqat", value: `${stats.attention}%`, icon: <Eye size={16} />, color: "#F5A623", chart: WEEK_DATA.map(d => ({ v: d.attention })) },
    { label: "Bu hafta", value: `${stats.week || 5} dars`, icon: <Activity size={16} />, color: "#7B2FBE", chart: [2,3,1,4,2,0,0].map(v => ({ v })) },
  ];

  return (
    <div className="min-h-full text-white p-4 md:p-6 xl:p-8 flex flex-col gap-6 relative z-10">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {QUICK_STATS.map((s, i) => (
          <motion.div key={s.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}
            className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 overflow-hidden relative">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          {/* Main Actions */}
          <section>
            <h2 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-3">Tezkor amallar</h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {MAIN_ACTIONS.map((a, i) => (
                <motion.div key={a.title} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 * i }}
                  ...
                  className={`relative rounded-2xl border cursor-pointer group transition-all duration-200 hover:-translate-y-1 overflow-hidden ${a.glow ? "border-red-500/40 shadow-lg shadow-red-500/10" : "border-white/10 hover:border-white/25"}`}
                  style={{ background: a.glow ? "rgba(232,72,85,0.08)" : "rgba(255,255,255,0.03)" }}
                  ...
                  <div className="p-4">
                    ...
                    <p className="font-bold text-sm text-white leading-tight">{a.title}</p>
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm font-bold text-slate-400 uppercase tracking-wider">So&apos;nggi darslar</h2>
              <Link href="/professor/lessons" className="text-xs text-slate-400 hover:text-[#F5A623] transition flex items-center gap-1 font-bold">
              ...
              {displayLessons.map((l, i) => {
                ...
                return (
                  <motion.div key={l.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 + i * 0.06 }}
                    className={`flex items-center gap-4 p-4 hover:bg-white/5 transition group ${i < displayLessons.length - 1 ? "border-b border-white/5" : ""}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-sm text-white truncate">{l.title}</p>
          ...
          {/* Week Chart */}
          <section>
            <h2 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-3">Haftalik faollik</h2>
                ...
                {WEEK_DATA.map((d, i) => (
                  <div key={d.day} className="flex-1 flex flex-col items-center gap-1">
                    <motion.div initial={{ height: 0 }} animate={{ height: d.lessons * 16 }} transition={{ delay: i * 0.05, duration: 0.4 }}
                      className="w-full rounded-t-md" style={{ background: d.lessons > 0 ? "#F5A623" : "rgba(255,255,255,0.05)", minHeight: 4 }} />
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={i} className="flex items-center gap-3">
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition ${goal.done ? "bg-[#0D9373] border-[#0D9373]" : "border-white/20"}`}>
                    {goal.done && <CheckCircle2 size={10} className="text-white" />}
                  </div>
                  <span className={`text-sm transition ${goal.done ? "line-through text-slate-500" : "text-slate-300"}`}>{goal.label}</span>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-2">
              {[
                ...
                <button key={item.href} onClick={() => router.push(item.href)}
                  className="w-full text-left px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 hover:border-white/25 hover:bg-white/10 transition text-sm font-bold flex items-center justify-between"
        ...
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-md p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              className="w-full max-w-sm rounded-3xl border border-white/10 bg-[#0F0F16] p-6 text-center shadow-2xl">
              ...
              <h3 className="text-xl font-bold text-white mb-2">Kirish talab qilinadi</h3>
              <p className="text-sm text-slate-400 mb-5">Bu amalni bajarish uchun tizimga kiring.</p>
